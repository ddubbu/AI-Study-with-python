...
C1 = tf.nn.conv2d(X, CW1, strides=[1, 1, 1, 1], padding="SAME")
...
print(P1.shape.as_list())  # [None, 14, 14, 2]
print(tf.shape(P1)[1])  # Tensor("strided_slice:0", shape=(), dtype=int32)
...
print("commonly, dim = 4 tensor.")
print("common shape = [(10개=) # train-example, (앞)height, (앞)width, (뒤) depth = channel]")
print("X:", sess.run(tf.shape(X), feed_dict={X: batch_xs}))
print("========layer 1=========")
print(" * CW1:", sess.run(tf.shape(CW1)), '= [height, width, in_channels, out_channels]')
print("C1 : ", sess.run(tf.shape(C1), feed_dict={X: batch_xs}))
print("P1 : ", sess.run(tf.shape(P1), feed_dict={X: batch_xs}))
print("========layer 2=========")
print("CW2:", sess.run(tf.shape(CW2), feed_dict={X: batch_xs}))
print("C2 : ", sess.run(tf.shape(C2), feed_dict={X: batch_xs}))
print("P2 : ", sess.run(tf.shape(P2), feed_dict={X: batch_xs}))
print("=======FC1 (layer 3)=======")
print("FC1 : ", sess.run(tf.shape(FC1), feed_dict={X: batch_xs}))
print("FC_W1 : ", sess.run(tf.shape(FC_W1), feed_dict={X: batch_xs}))
print("L3 : ", sess.run(tf.shape(L3), feed_dict={X: batch_xs}))
print("=======FC2 (layer 3)=======")
print("FC_W2 : ", sess.run(tf.shape(FC_W2), feed_dict={X: batch_xs}))
print("L4 : ", sess.run(tf.shape(L4), feed_dict={X: batch_xs}))

print("=======(The End - Shape TEST)=======")
sess.close()
...
with tf.Session() as sess:
    ...
    print("Testing Accuracy:",
          sess.run(metric, feed_dict={X: mnist.test.images.reshape(-1, 28, 28, 1),  # ★
                                      Y: mnist.test.labels}))
